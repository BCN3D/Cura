...
class SliceInfo(Extension):
    ...
    def __init__(self):
        ...
        if not Preferences.getInstance().getValue("info/asked_send_slice_info"):
            self.send_slice_info_message = Message(catalog.i18nc("@info", "BCN3D Cura collects anonymized usage statistics."),
                                                   lifetime = 0,
                                                   dismissable = False,
                                                   title = catalog.i18nc("@info:title", "Collecting Data"))

            self.send_slice_info_message.addAction("Dismiss", name = catalog.i18nc("@action:button", "Allow"), icon = None,
                    description = catalog.i18nc("@action:tooltip", "Allow BCN3D Cura to send anonymized usage statistics to help prioritize future improvements to BCN3D Cura. Some of your preferences and settings are sent, the BCN3D Cura version and a hash of the models you're slicing."))
            self.send_slice_info_message.addAction("Disable", name = catalog.i18nc("@action:button", "Disable"), icon = None,
                    description = catalog.i18nc("@action:tooltip", "Don't allow BCN3D Cura to send anonymized usage statistics. You can enable it again in the preferences."), button_style = Message.ActionButtonStyle.LINK)
    ...
    def _onWriteStarted(self, output_device):
        try:
            ...
            for extruder in extruders:
                ...
                for key in extruder.getAllKeys():
                    if extruder.getProperty(key, "settable_per_extruder"):
                        extruder_settings[key] = extruder.getProperty(key, "value")
            ...
            for key in global_container_stack.getAllKeys():
                if not global_container_stack.getProperty(key, "settable_per_extruder"):
                    print_settings[key] = global_container_stack.getProperty(key, "value")
    ...
    def _writeToGCode(self, json_data):
        ...
        if not any(";PrintInfo/" in s for s in gcode_list):
            gcode_list.append("\n\n;PrintInfo/" + str(json_data) + "/PrintInfo\n")